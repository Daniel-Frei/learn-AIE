...
export const L5_DeepReinforcementLearning: Question[] = [
...

  {
    ...
    options: [
      ...
      { text: "It must sample actions from a Gaussian distribution parameterized by a mean and variance.", isCorrect: false },
      { text: "It outputs a probability distribution only after applying a softmax to ensure outputs sum to one.", isCorrect: false },
      { text: "It can only choose actions that are labeled by humans during training.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "A single forward pass can produce \\(Q(s,a)\\) values for all discrete actions if the network is parameterized to output a vector.", isCorrect: true },
      { text: "Choosing the greedy action means selecting the index of the largest output Q-value.", isCorrect: true },
      { text: "The Q-values can be interpreted as estimates of expected discounted return for each action from that state.", isCorrect: true },
      { text: "Because the network outputs multiple values, the outputs must sum to 1 like probabilities.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about action spaces are correct?",
    options: [
...

  {
    ...
    options: [
      { text: "It may fail to explore alternative actions, making it harder to discover higher-reward strategies.", isCorrect: true },
      { text: "It guarantees optimal performance in any stochastic environment.", isCorrect: false },
      { text: "It eliminates the need for a reward function.", isCorrect: false },
      { text: "It makes discounting \\(\\gamma\\) unnecessary in all tasks.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "A Gaussian policy can be parameterized by a mean \\(\\mu(s)\\) and variance (or standard deviation) \\(\\sigma^2(s)\\).", isCorrect: true },
      { text: "Sampling from \\(\\mathcal{N}(\\mu(s),\\sigma^2(s))\\) produces a continuous action value.", isCorrect: true },
      { text: "Predicting \\(\\mu\\) and \\(\\sigma\\) avoids needing to output an infinite number of probabilities for every possible action.", isCorrect: true },
      { text: "A Gaussian policy can only be used for discrete actions like \\{left,right\\}.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "The state (or observation) should contain enough information for the agent to decide what to do next.", isCorrect: true },
      { text: "In Atari-style settings, the state might be represented by raw pixels (frames) from the screen.", isCorrect: true },
      { text: "The state is always a single scalar number (e.g., the agent’s x-position).", isCorrect: false },
      { text: "The state never changes in response to actions.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about a typical Q-learning target are correct?",
    options: [
      { text: "A common one-step target is \\(y_t = r_t + \\gamma \\max_{a'} Q(s_{t+1}, a')\\).", isCorrect: true },
      { text: "The Q-loss often measures a difference between a predicted value \\(Q(s_t,a_t)\\) and a target value \\(y_t\\), e.g., via squared error.", isCorrect: true },
      { text: "The target includes both the immediate reward and a discounted estimate of future value.", isCorrect: true },
      { text: "The Q-loss cannot be optimized with backpropagation because it contains a max operator.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about exploration are correct?",
    options: [
      { text: "Sampling from a stochastic policy can lead to trying actions that are not currently the highest-probability choice.", isCorrect: true },
      { text: "Exploration can help discover strategies that have higher long-term return than the agent’s current behavior.", isCorrect: true },
      { text: "Exploration is unnecessary whenever rewards are sparse and delayed.", isCorrect: false },
      { text: "Exploration only matters in supervised learning, not in reinforcement learning.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "Because \\(R_t\\) includes future rewards, an action with lower immediate reward can still lead to higher total discounted return.", isCorrect: true },
      { text: "Because \\(R_t\\) ignores all rewards after time \\(t\\).", isCorrect: false },
      { text: "Because \\(R_t\\) is defined as \\(R_t = r_t\\) for all tasks.", isCorrect: false },
      { text: "Because maximizing return forces the environment to become deterministic.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "A common loss form is \\(\\mathcal{L} = -\\log \\pi_\\theta(a_t\\mid s_t)\\, R_t\\) (or using an advantage estimate).", isCorrect: true },
      { text: "If \\(R_t\\) is high, gradient updates tend to increase the probability of the taken action under \\(\\pi_\\theta\\).", isCorrect: true },
      { text: "If \\(R_t\\) is low (or negative), gradient updates tend to decrease the probability of the taken action under \\(\\pi_\\theta\\).", isCorrect: true },
      { text: "Policy gradient requires computing \\(\\arg\\max_a Q(s,a)\\) at every step.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "An episode can be rolled out by repeatedly sampling actions from \\(\\pi_\\theta(\\cdot\\mid s_t)\\) until termination.", isCorrect: true },
      { text: "You can store a sequence of \\((s_t,a_t,r_t)\\) and later compute returns for training.", isCorrect: true },
      { text: "If reward only arrives at the end, credit assignment becomes harder because many earlier actions share the same final outcome.", isCorrect: true },
      { text: "Because episodes end, discounting \\(\\gamma\\) is mathematically impossible.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "In real driving, 'run until termination' can be unsafe because termination might correspond to a crash.", isCorrect: true },
      { text: "High-fidelity simulators can allow safe rollouts where termination is acceptable, and learned policies may later transfer to the real world.", isCorrect: true },
      { text: "A major practical challenge is collecting enough diverse experience without causing real-world harm.", isCorrect: true },
      { text: "Simulation is unnecessary because real-world trial-and-error is always cheaper and safer.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "It is the set of actions the agent is allowed to take in the environment (which may be discrete or continuous).", isCorrect: true },
      { text: "It is the set of all possible future trajectories of the agent over its entire lifetime.", isCorrect: false },
      { text: "It is the set of rewards \\(r_t\\) the environment can output.", isCorrect: false },
      { text: "It is the set of network weights \\(\\theta\\) used to represent the policy.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "The term \\(\\log \\pi_\\theta(a_t\\mid s_t)\\) increases when the policy assigns higher probability to the taken action.", isCorrect: true },
      { text: "Using \\(-\\log \\pi_\\theta(a_t\\mid s_t)\\,R_t\\) ties probability updates to how good the observed return was.", isCorrect: true },
      { text: "If an action was unlikely under the current policy but led to high return, policy gradient can still increase its probability.", isCorrect: true },
      { text: "The log-likelihood is used only in Q-learning losses, not in policy learning.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "A 19x19 Go board leads to an enormous space of possible positions and long-horizon consequences.", isCorrect: true },
      { text: "Self-play allows an agent to generate its own training experience by playing against itself.", isCorrect: true },
      { text: "A value estimate for a board position helps evaluate 'how good the situation is' without waiting until the end of the game.", isCorrect: true },
      { text: "Because Go is complex, reinforcement learning cannot be used at all and only supervised learning works.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "Each output probability should be non-negative.", isCorrect: true },
      { text: "The probabilities across discrete actions should sum to 1.", isCorrect: true },
      { text: "A softmax transformation is a common way to map arbitrary logits to a valid probability distribution.", isCorrect: true },
      { text: "Those constraints are necessary because Q-values must sum to 1.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "If a Q-network takes \\((s,a)\\) as input, you may need to evaluate it once per candidate action to compare actions.", isCorrect: true },
      { text: "If a Q-network outputs a vector of Q-values for all actions given \\(s\\), you can get all action values in a single forward pass.", isCorrect: true },
      { text: "Reducing the number of forward passes can matter because inference cost can be a bottleneck, especially in real-time systems.", isCorrect: true },
      { text: "Inference cost is irrelevant because only backpropagation is computationally expensive.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "A policy \\(\\pi\\) defines how actions are selected given states (deterministically or stochastically).", isCorrect: true },
      { text: "A value function (like \\(Q\\)) evaluates how good actions or states are in terms of expected return.", isCorrect: true },
      { text: "You can derive a greedy policy from a Q-function via \\(\\pi(s)=\\arg\\max_a Q(s,a)\\) in discrete action settings.", isCorrect: true },
      { text: "A policy and a value function are identical objects and always have the same output type.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "An agent can be initialized via supervised learning from expert games to learn plausible moves from board states.", isCorrect: true },
      { text: "Reinforcement learning via self-play can improve the agent beyond the supervised imitation of humans.", isCorrect: true },
      { text: "A value estimate can reduce reliance on sparse end-of-game rewards by evaluating intermediate positions.", isCorrect: true },
      { text: "Self-play is incompatible with neural networks because it requires symbolic search only.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about stochastic policies are correct?",
    options: [
...

  {
    ...
    options: [
      { text: "If an agent must consider many actions, computing \\(\\arg\\max_a Q(s,a)\\) can become expensive.", isCorrect: true },
      { text: "Even when per-step legal actions are manageable, exploring multi-step futures can explode combinatorially.", isCorrect: true },
      { text: "Large action/state spaces often require function approximation (e.g., neural networks) rather than tabular methods.", isCorrect: true },
      { text: "Large action spaces make the environment’s reward function unnecessary.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    options: [
      { text: "Value learning with greedy action selection can be fully deterministic given a fixed \\(Q\\) estimate.", isCorrect: true },
      { text: "Policy learning naturally supports stochastic policies by modeling \\(\\pi(a\\mid s)\\) and sampling actions.", isCorrect: true },
      { text: "One motivation for policy learning is handling continuous actions without enumerating all actions.", isCorrect: true },
      { text: "Policy learning cannot be trained with gradient descent because sampling breaks differentiability completely.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about discounted return \\(R_t\\) are correct?",
    options: [
      { text: "If \\(0<\\gamma<1\\), the contribution of rewards decays geometrically with how far in the future they occur.", isCorrect: true },
      { text: "If all rewards are bounded, using \\(\\gamma<1\\) helps keep the infinite-horizon sum \\(\\sum_{k=0}^{\\infty} \\gamma^k r_{t+k}\\) finite.", isCorrect: true },
      { text: "Discounting is one reason why an agent may prioritize near-term outcomes over equally-sized far-future outcomes.", isCorrect: true },
      { text: "Discounting implies the agent ignores all future rewards beyond the next step.", isCorrect: false },
    ],
    explanation:
...

  {
    ...
    difficulty: "medium",
    prompt:
      "Which statements about 'self-play from scratch' are correct?",
    options: [
      { text: "Self-play can generate training data without human demonstrations by having the agent play against itself.", isCorrect: true },
      { text: "Learning from scratch typically starts from a weak or random policy and improves via repeated interaction and feedback.", isCorrect: true },
      { text: "Adding a value estimate can help learning by providing a signal about intermediate positions, not only final outcomes.", isCorrect: true },
      { text: "Self-play requires that the reward be dense at every time step; otherwise it cannot learn.", isCorrect: false },
    ],
    explanation:
