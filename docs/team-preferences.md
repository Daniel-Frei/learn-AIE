...
- Quiz prompts may use compact GitHub-Flavored Markdown tables when tabular data, scenarios, cases, probability values, model states, trial arms, or calculation inputs would be easier to read that way. Tables should be introduced by the prompt, use meaningful row and column labels, stay mobile-friendly, and remain self-contained.
