...
- When a source bundle includes both slides and a transcript, use the slides' teaching time, worked sequences, and visual emphasis to set the high-level question allocation; use the transcript for nuance and explanation depth. Do not give every named technique an equal quota or let a brief transcript aside outweigh the deck's central mechanisms.
- Stanford CME296 source bundles should follow the Stanford CME295 storage pattern: keep paired `lecture N - slides.pdf` and `lecture N - transcript.md` files under the course's `transcripts-and-files` folder.
- When CME296 questions are scoped to a paper-specific study guide, treat 60 questions as the budget for a complete lecture and scale the count in proportion to the selected slide coverage. Do not fill the remaining budget with material outside the recommended ranges, and do not register low-priority lectures merely to complete the sequence.
- Diffusion-paper presentations are aimed at a technical paper-club audience that already understands transformers and LLMs but may be new to diffusion. Teach diffusion and relevant flow-matching intuition in plain language before paper-specific notation, keep the core LLM connection explicit, and target roughly 45-55 minutes of prepared material so a one-hour session retains discussion time.
- Paper-club presentations should open with a concrete, source-grounded tension, surprising behavior, or result that gives the audience a reason to want the explanation. The opening should create curiosity without overstating the paper or resorting to literal clickbait.
